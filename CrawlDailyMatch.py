...
def daily_process(date):
    ...
    count = 0  # 进度计数器
    url_daily_match = my_crawl.url_daily_match % date
...
def get_gameid(url_daily_match):
    raw_data = my_crawl.get_raw_data(url_daily_match)
    game_id = []
    try:
...
def get_daily_match_info(each):
    url_daily_snapshot = my_crawl.url_daily_snapshot % each
    raw_data = my_crawl.get_raw_data(url_daily_snapshot)
...
def resol_daily_match_data(raw_data, url):
    ...
    resol_data['gameTime'] = my_crawl.get_game_time(data['gameProfile']['utcMillis'])
...
def save_daily_match(data):
    # 判断数据库中是否已经存在对应数据
    if my_crawl.collection1.count_documents({'url': data['url']}) == 0:
        my_crawl.collection1.insert_one(data)
    else:
        if my_crawl.collection1.find_one({'url': data['url']})['timestamp'] == data['timestamp']:
            print('\n%s基础数据重复爬取\n' % data['gameId'])
        else:
            my_crawl.collection1.update_one({'url': data['url']}, {'$set': data})
...
def save_match_list(data):
    if my_crawl.collection2.count_documents({'url': data['url']}) == 0:
        my_crawl.collection2.insert_one(data)
    else:
        if my_crawl.collection2.find_one({'url': data['url']})['timestamp'] == data['timestamp']:
            print('\n%s比赛列表数据重复爬取\n' % data['date'])
        else:  # 数据更新
            my_crawl.collection2.update_one({'url': data['url']}, {'$set': data})
...
def get_playByPlays_info(each, period):
    ...
    for i in range(1, int(period) + 1):
        url_playByPlays = my_crawl.url_playByPlays % (each, str(i))
        raw_data = my_crawl.get_raw_data(url_playByPlays)  # 已包含延时
...
def resol_playByPlays_data(raw_data, i, playByPlays_data):
    ...
    for each in data:
        if each['messageType'] in my_crawl.mtlist:
            my_crawl.mtlist.remove(each['messageType'])
...
def save_playByPlays(data):
    if my_crawl.collection3.count_documents({'gameId': data['gameId']}) == 0:
        my_crawl.collection3.insert_one(data)
    else:
        print('\n%s文字直播数据重复爬取\n' % data['gameId'])
    return ""
